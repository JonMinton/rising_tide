...
all_corrblocks <- 
  crossing(
    Country = c("France", "Japan", "United Kingdom", "USA", "United Kingdom"),
    sex = c("female", "male"),
...
add_format_worksheet <- function(wb, label, corr_data){
  wb %>% addWorksheet(sheetName = label)
  ...
  wb %>% conditionalFormatting(sheet = label, cols = 1:ncol(corr_data), rows = 1:(nrow(block_01)),
                             type = "colorscale", style = c("red", "white", "blue"), rule = c(-1, 0, 1))
